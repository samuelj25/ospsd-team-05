...
class TestProperties:
    ...
    def test_id(self) -> None:
        ...
        evt = event_impl.GoogleCalendarEvent(_make_raw())
...
class TestDefaults:
    ...
    def test_location_defaults_to_none(self) -> None:
        ...
        evt = event_impl.GoogleCalendarEvent(raw)
...
class TestJsonStringInput:
    ...
    def test_from_json_string(self) -> None:
        ...
        raw = _make_raw()
        evt = event_impl.GoogleCalendarEvent(json.dumps(raw))
        assert evt.id == "evt-123"
        assert evt.title == "Test Event"
...
class TestValidation:
    ...
    def test_missing_id_raises(self) -> None:
        ...
        with pytest.raises(TypeError, match="'id'"):
            event_impl.GoogleCalendarEvent(raw)
    ...
    def test_invalid_json_string_raises(self) -> None:
        ...
        with pytest.raises(ValueError, match=r"(?i)json"):
            event_impl.GoogleCalendarEvent("not valid json {{{")
...
class TestAbstractContract:
    """Verify GoogleCalendarEvent satisfies the Event ABC."""

    def test_isinstance_of_event(self) -> None:
        """GoogleCalendarEvent should be recognized as an Event instance."""
        evt = event_impl.GoogleCalendarEvent(_make_raw())
